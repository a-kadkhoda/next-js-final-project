...
const AddToCart = (props: IProps) => {
  ...
  return (
    <>
      {name == "" ? (
        <span className="text-3xl text-warning font-bold border p-10 text-center">
          You Have To Loggin First
        </span>
      ) : (
        <button
          onClick={() => dispatch(setToCart(id))}
          className={`btn ${cart.includes(id) ? "btn-error" : "btn-success"}`}
        >
